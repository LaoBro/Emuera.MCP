...
async function mountWith(lines: DisplaySnapshot['lines'][number][], bgImages: BgImageState[] = []) {
...
describe('TerminalDisplay: image 掩膜渲染（Q6）', () => {
  ...
  it('掩膜高度钉死=行高（CSS 变量），overflow 可见', async () => {
    const { wrapper } = await mountWith([line([imgSeg()])]);
    const mask = wrapper.find('.term-mask');
  ...
  it('ypos 溢出：img margin-top = ypos（可为负）', async () => {
