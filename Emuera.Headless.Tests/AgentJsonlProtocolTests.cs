...
public class AgentJsonlProtocolTests : IDisposable
{
    ...
    public async Task T_buildturn_tinput_populates_timer_fields()
    {
        ...
        Assert.Equal(9, root.GetProperty("protocolVersion").GetInt32()); // issue 07：v8 → v9
    ...
    public async Task T_buildturn_initial_includes_protocol_version()
    {
        ...
        Assert.NotNull(json);
        using var doc = JsonDocument.Parse(json!);
        Assert.Equal(9, doc.RootElement.GetProperty("protocolVersion").GetInt32()); // issue 07：v8 → v9
    }
